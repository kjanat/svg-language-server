...
`compute_compat_verdict` runs **at build time** in `build.rs` for every catalog entry, per snapshot. The output is baked into the generated catalog as `verdict_by_profile: &'static [(SpecSnapshotId, CompatVerdict)]` (a small array, 4 profiles × 2 bytes + reason slice pointer). Runtime hover/lint simply look up the verdict for the currently-selected profile and render it — zero computation in the hot path.
...
When any browser on the current entry carries `version_removed`, `partial_implementation`, `prefix`, `notes`, `alternative_name`, or `flags`, `BrowserNotes` gets populated with a bulleted list. Example rendering:
...
Each sub-bullet is generated from the new fields on `BrowserVersion`. If nothing would be in the list, the whole `BrowserNotes` section is omitted.
...
**Legacy-alias caveat**: not every "modern replacement" should be modeled as deprecation or removal. `font-stretch` is the sharp edge here.

- Current repo snapshots still treat `font-stretch` as present and normal in SVG 1.1 and SVG 2, and generated `svg-data` currently bakes it as `spec_lifecycle: Stable`, `deprecated: false`.
- SVG 2 styling still lists `font-stretch` among presentation attributes and required CSS properties: `https://svgwg.org/svg2-draft/styling.html`.
- CSS Fonts 3 still defines section `3.3. Font width: the font-stretch property`: `https://www.w3.org/TR/css-fonts-3/#font-stretch-prop`.
- Newer sources disagree on framing, not behavior. MDN says: `The font-stretch attribute was renamed to font-width ... the specification retains font-stretch as a legacy alias`; CSS Fonts 4 says: `For historical reasons, a font-stretch property exists ... and functions in the identical way to the font-width.`
- Implication for this plan: `font-stretch` should not be force-fit into the four deprecation-audit buckets above. It is an alias / rename discrepancy across spec generations, not a straightforward `Deprecated` or `Obsolete` case like `xlink:href`, `baseProfile`, or `version`.
- If we decide to surface this in hover/lint/completion, that likely wants new data fields such as `replacement` / `legacy_alias_of` rather than overloading `deprecated: true`.
