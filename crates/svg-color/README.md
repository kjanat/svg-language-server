...
- Extract color values from parsed SVG trees (hex, modern and legacy
  `rgb()`/`hsl()`, `hwb()`, `lab()`, `lch()`, `oklab()`, `oklch()`, named
  colors)
- Extract colors from SVG presentation attributes and embedded `<style>` blocks
- Resolves CSS custom properties and `color-mix(...)` inside embedded `<style>`
  blocks
...
Embedded CSS can also resolve derived colors such as `fill: var(--toolbar-bg)`
and `color-mix(in oklch, var(--base), white 8%)`.
