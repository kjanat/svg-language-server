package io.github.treesitter.jtreesitter.svgpaint;

import java.lang.foreign.*;

public final class TreeSitterSvgPaint {
    ...
    private static final TreeSitterSvgPaint INSTANCE = new TreeSitterSvgPaint();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterSvgPaint() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_svg_paint");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-svg-paint");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-svg-paint");
