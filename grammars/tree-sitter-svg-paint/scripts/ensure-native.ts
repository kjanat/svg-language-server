 ...
 *
 *   prebuilds/<platform>-<arch>/tree-sitter-svg-paint.node
 *
 ...
 * tree-sitter's own install), so this only builds the paint sub-grammar's addon.
 */
...
try {
	...
	ensureAddon(grammarRoot, 'tree_sitter_svg_paint_binding.node', 'tree-sitter-svg-paint');
} catch (error) {
