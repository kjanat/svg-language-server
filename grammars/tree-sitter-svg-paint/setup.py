...
class Build(build):
    ...
    def run(self) -> None:
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_svg_paint", "queries")
...
_ = setup(
    ...
    package_data={
        "tree_sitter_svg_paint": ["*.pyi", "py.typed"],
        "tree_sitter_svg_paint.queries": ["*.scm"],
    },
    ext_package="tree_sitter_svg_paint",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_svg_paint/binding.c",
                "src/parser.c",
            ],
            define_macros=[
