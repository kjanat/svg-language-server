package io.github.treesitter.jtreesitter.svgpath;

import java.lang.foreign.*;

public final class TreeSitterSvgPath {
    ...
    private static final TreeSitterSvgPath INSTANCE = new TreeSitterSvgPath();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterSvgPath() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_svg_path");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-svg-path");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-svg-path");
