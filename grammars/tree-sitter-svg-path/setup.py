...
class Build(build):
    ...
    def run(self) -> None:
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_svg_path", "queries")
...
_ = setup(
    ...
    package_data={
        "tree_sitter_svg_path": ["*.pyi", "py.typed"],
        "tree_sitter_svg_path.queries": ["*.scm"],
    },
    ext_package="tree_sitter_svg_path",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_svg_path/binding.c",
                "src/parser.c",
            ],
            define_macros=[
