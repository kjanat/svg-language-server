package io.github.treesitter.jtreesitter.svgtransform;

import java.lang.foreign.*;

public final class TreeSitterSvgTransform {
    ...
    private static final TreeSitterSvgTransform INSTANCE = new TreeSitterSvgTransform();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterSvgTransform() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_svg_transform");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-svg-transform");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-svg-transform");
