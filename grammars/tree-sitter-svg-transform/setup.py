...
class Build(build):
    ...
    def run(self) -> None:
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_svg_transform", "queries")
...
_ = setup(
    ...
    package_data={
        "tree_sitter_svg_transform": ["*.pyi", "py.typed"],
        "tree_sitter_svg_transform.queries": ["*.scm"],
    },
    ext_package="tree_sitter_svg_transform",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_svg_transform/binding.c",
                "src/parser.c",
            ],
            define_macros=[
