...
class Build(build):
    ...
    def run(self) -> None:
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_svg", "queries")
...
_ = setup(
    ...
    package_data={
        "tree_sitter_svg": ["*.pyi", "py.typed"],
        "tree_sitter_svg.queries": ["*.scm"],
    },
    ext_package="tree_sitter_svg",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_svg/binding.c",
                "src/parser.c",
            ],
            define_macros=[
