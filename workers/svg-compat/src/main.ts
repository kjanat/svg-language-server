...
<body>
  <main>
    <div class="hero">
      ...
      <div class="muted">Generated ${escape(output.generated_at)}.<br>
        ...
        <code>?bcd=</code><span class="ver-wrap"><input type="text" class="ver" id="bcd-ver" placeholder="version" autocomplete="off" aria-label="BCD version"><ul class="ver-list" id="bcd-list" hidden></ul></span><code>&amp;wf=</code><span class="ver-wrap"><input type="text" class="ver" id="wf-ver" placeholder="version" autocomplete="off" aria-label="Web Features version"><ul class="ver-list" id="wf-list" hidden></ul></span>.
      </div>
      <p>
  ...
  <script>
    const fields = [
      { id: "bcd-ver", listId: "bcd-list", pkg: "@mdn/browser-compat-data" },
      { id: "wf-ver",  listId: "wf-list",  pkg: "web-features" },
    ];
    function navigate() {
    ...
    for (const f of fields) {
      ...
      input.setAttribute("aria-controls", f.listId);
      list.setAttribute("role", "listbox");
      fetch("https://data.jsdelivr.com/v1/package/npm/" + f.pkg)
        .then(r => r.json())
        .then(data => { allVersions = data.versions; })
        .catch(() => {});
      function show(filter) {
        ...
        const matches = q
          ? allVersions.filter(v => v.includes(q)).sort((a, b) => {
        ...
        for (const v of matches.slice(0, 100)) {
          ...
          li.onmousedown = e => { e.preventDefault(); pick(v); };
          list.appendChild(li);
        }
      ...
      input.addEventListener("keydown", e => {
        ...
        if (e.key === "ArrowDown" || (e.key === "Tab" && !e.shiftKey && !list.hidden)) {
          e.preventDefault();
          if (list.hidden) show(input.value);
          else highlight(active + 1);
        } else if (e.key === "ArrowUp" || (e.key === "Tab" && e.shiftKey && !list.hidden)) {
          e.preventDefault();
          highlight(active - 1);
